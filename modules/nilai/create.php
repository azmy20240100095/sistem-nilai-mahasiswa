...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($errors)) {
        $valid = validate_nilai_unique($conn, $mahasiswa_id, $mata_kuliah_id);
        if ($valid !== true) $errors[] = $valid;
    }
    ...
    if (empty($errors)) {
        ...
        $stmt = $conn->prepare("INSERT INTO nilai (mahasiswa_id, mata_kuliah_id, nilai, grade) VALUES (?, ?, ?, ?)");
        $stmt->bind_param("iids", $mahasiswa_id, $mata_kuliah_id, $nilai, $grade);
        
        if ($stmt->execute()) {
            set_alert('success', 'Data nilai berhasil ditambahkan!');
            redirect('index.php');
        } else {
            set_alert('danger', 'Gagal menambahkan data nilai!');
        }
    }
}
?>
    ...
    <div class="row">
        ...
        <div class="col-md-10 content">
            <div class="content-header">
                <h2><i class="bi bi-plus-circle"></i> Tambah Nilai</h2>
                <p class="text-muted">Form tambah data nilai mahasiswa</p>
            </div>
                    ...
                    <form method="POST" action="">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="mahasiswa_id" name="mahasiswa_id" required>
                                ...
                                <?php while ($mhs = $mahasiswa_result->fetch_assoc()): ?>
                                    <option value="<?php echo $mhs['id']; ?>" <?php echo (isset($_POST['mahasiswa_id']) && $_POST['mahasiswa_id'] == $mhs['id']) ? 'selected' : ''; ?>>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="mata_kuliah_id" name="mata_kuliah_id" required>
                                ...
                                <?php while ($mk = $matkul_result->fetch_assoc()): ?>
                                    <option value="<?php echo $mk['id']; ?>" <?php echo (isset($_POST['mata_kuliah_id']) && $_POST['mata_kuliah_id'] == $mk['id']) ? 'selected' : ''; ?>>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control" id="nilai" name="nilai" value="<?php echo isset($_POST['nilai']) ? $_POST['nilai'] : ''; ?>" min="0" max="100" step="0.01" required>
